...
同梱する外部ライブラリは、すべて Ruby だけで書かれたものです。
Ruby のバージョンごとに分けず単一のディレクトリにまとめてあり、tDiary が対応する Ruby 3.3、3.4、4.0 のいずれでもそのまま読み込めます。

C 拡張を持つライブラリは同梱しません。
日付処理の date は、Ruby 本体に同梱される default gem を使います。
cgi は同梱しますが、文字列のエスケープには C 拡張を含めず、Ruby 本体が提供する実装か Ruby で書かれた実装を使うため、展開先でのコンパイルは不要です。
