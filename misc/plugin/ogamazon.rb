...
def ogamazon_fetch( url, limit = 10 )
	...
	when Net::HTTPRedirection
		ogamazon_fetch( res['location'].untaint, limit - 1 )
...
def ogamazon_call_ecs( asin, id_type, country )
...
def ogamazon_author( item )
	begin
		author = []
		%w(Author Creator Artist).each do |elem|
			item.xpath( "*/#{elem}" ) do |a|
				author << a.text
			end
		end
...
def ogamazon_title( item )
	@conf.to_native( item.xpath( '*/Title' )[0].text, 'utf-8' )
end

def ogamazon_image( item )
	image = {}
	begin
		...
		img = item.xpath("#{size}Image")[0] || item.xpath("ImageSets/ImageSet/#{size}Image")[0]
		image[:src] = img.xpath('URL').text
		image[:src].gsub!(/http:\/\/ecx\.images-amazon\.com/, 'https://images-na.ssl-images-amazon.com')
		image[:height] = img.xpath('Height').text
		image[:width] = img.xpath('Width').text
	rescue
...
def ogamazon_url( item )
	item.xpath( 'DetailPageURL' )[0].text
end

def ogamazon_label( item )
	begin
		@conf.to_native( item.xpath( '*/Label' )[0].text, 'utf-8' )
	rescue
		'-'
	end
end

def ogamazon_price( item )
	begin
		@conf.to_native( item.xpath( '*/LowestNewPrice/FormattedPrice' )[0].text, 'utf-8' )
	rescue
		begin
			@conf.to_native( item.xpath( '*/ListPrice/FormattedPrice' )[0].text, 'utf-8' )
		rescue
			'(no price)'
		end
	end
...
def ogamazon_detail_html( item )
	author = ogamazon_author( item )
	title = ogamazon_title( item )

	size_orig = @conf['amazon.imgsize']
	@conf['amazon.imgsize'] = 2
	image = ogamazon_image( item )
	@conf['amazon.imgsize'] = size_orig

	url = ogamazon_url( item )
	<<-HTML
	<a class="amazon-detail" href="#{url}"><span class="amazon-detail">
		...
		<span class="amazon-detail-desc">
			...
			<span class="amazon-label">#{h ogamazon_label( item )}</span><br>
			<span class="amazon-price">#{h ogamazon_price( item )}</span>
		</span><br style="clear: left">
	</span></a>
	HTML
end

def ogamazon_to_html( item, with_image = true, label = nil, pos = 'amazon' )
	with_image = false if @mode == 'categoryview'

	author = ogamazon_author( item )
	author = "(#{author})" unless author.empty?

	label ||= %Q|#{ogamazon_title( item )}#{author}|
	alt = ''
...
def ogamazon_get( asin, with_image = true, label = nil, pos = 'amazon' )
	...
	begin
		...
		begin
			...
			xml = ogamazon_call_ecs( asin, id_type, country )
		...
		if pos == 'detail' then
			...
			ogamazon_to_html( item, with_image, label, pos )
		end
		...
		if @mode == 'preview' then
			if item == nil then
				m = doc.xpath( 'Items/Request/Errors/Error/Message' )[0].text
...
def isbn_detail( asin )
	ogamazon_get( asin, true, nil, 'detail' )
end

def isbn_image( asin, label = nil )
	ogamazon_get( asin, true, label )
end

def isbn_image_left( asin, label = nil )
	ogamazon_get( asin, true, label, 'left' )
end

def isbn_image_right( asin, label = nil )
	ogamazon_get( asin, true, label, 'right' )
end

def isbn( asin, label = nil )
	ogamazon_get( asin, false, label )
end
