...
def system_update
	...
	begin
		...
		pull_request = client.create_pull_request("#{user}/tdiary-core", 'main', 'tdiary:main', title, body)
...
add_conf_proc('system_update', 'システム更新', 'basic') do
	...
	r = <<-HTML
		...
		<p>GitHubとHerokuの連携機能を使って、tDaiaryシステムの更新を行います。あらかじめ<a href="https://github.com/tdiary/tdiary-core">tDiaryの公式リポジトリ</a>を自分のアカウントにforkし、そのリポジトリをHerokuのGitHub Deployに指定しておく必要があります。更新はmainブランチを使って行われます。</p>
