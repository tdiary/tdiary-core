...
_HTML
	...
	add_header_proc do
		if not @conf.bot? and not @cgi.mobile_agent? then
	...
	add_section_leave_proc do
		r = ''
		if @my_sequel_active and @my_sequel_date and @my_sequel_anchor and not @conf.bot? and not @cgi.mobile_agent? then
